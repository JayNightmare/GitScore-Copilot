...
export default function Home() {
  ...
  const handleSubmit = async (e) => {
    ...
    const match = input.trim().match(/(?:https?:\/\/github\.com\/)?([^\/]+)\/([^\/\s]+)/);
    if (!match) {
      setError('Please enter a valid repository in format "owner/repo" or GitHub URL');
      return;
    }
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Analyze and score GitHub repositories based on quality metrics" />
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div className="text-center">
              ...
              <p className="text-lg text-github-600">
                Analyze repository quality with AI-powered scoring
              </p>
            </div>
          </div>
        </header>
              ...
              <div className="score-card">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <p className="text-github-600 mb-2">{result.repository.description}</p>
              ...
              <div className="score-card text-center">
                ...
                <div className={`text-6xl font-bold mb-2 ${getScoreColor(result.score)}`}>
                  <CountUp
                    ...
                    decimal="."
                  />
                  <span className="text-3xl text-github-400">/10</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-4 mb-4">
                  <div
                    className={`h-4 rounded-full transition-all duration-1000 ease-out ${getScoreBackground(result.score)}`}
                ...
                <p className="text-gray-600">
                  {result.score >= 8 ? 'Excellent' : 
                   result.score >= 6 ? 'Good' : 
                   result.score >= 4 ? 'Fair' : 'Needs Improvement'}
                </p>
              </div>
                  ...
                  {Object.entries(result.breakdown).map(([key, metric]) => (
                    <div key={key} className="metric-item">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-github-500 mt-1">{metric.details}</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <footer className="bg-white border-t border-gray-200 mt-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="text-center text-github-500">
              <p>Built with Next.js, Tailwind CSS, and GitHub GraphQL API</p>
              <p className="mt-2">
                Open source project for analyzing repository quality metrics
              </p>
            </div>
          </div>
        </footer>
