...
function scoreLicense(licenseInfo) {
  ...
  const popularLicenses = ['mit', 'apache-2.0', 'gpl-3.0', 'bsd-3-clause', 'bsd-2-clause'];
...
function scoreCommits(commitHistory) {
  ...
  const recentActivity = recentCommits.filter(commit => 
    new Date(commit.committedDate) > thirtyDaysAgo
  ).length;
