 ...
 * 
 * Scoring Categories:
 * - Documentation (20%): README, CONTRIBUTING, CoC, Changelog
 ...
 * Evaluates README quality, CONTRIBUTING guide, Code of Conduct, and Changelog
 */
function scoreDocumentation(repo, readmeText, contributingText, changelogText) {
    ...
    const avgScore = (scores.readme + scores.contributing + scores.codeOfConduct + scores.changelog) / 4;
    
    return {
        score: avgScore,
        details: `README: ${readmeScore.toFixed(1)}, CoC: ${scores.codeOfConduct}, Releases: ${scores.changelog}`,
        breakdown: scores
    };
}
...
function scoreCommunity(repo, commitHistory) {
    ...
    return {
        score: avgScore,
        details: `${contributorCount} contributors, Discussions: ${repo.hasDiscussionsEnabled ? 'Yes' : 'No'}`,
        breakdown: scores
    };
}
...
function calculateRepositoryScore(repoData, workflowData) {
    ...
    const changelogText = repo.changelogObject?.text;
    const commitHistory = repo.defaultBranchRef?.target?.history;

    // Calculate scores for each category (each returns 0-1)
    const documentation = scoreDocumentation(repo, readmeText, contributingText, changelogText);
