      ...
      defaultBranchRef: {
        target: {
          history: {
            totalCount: 150,
            nodes: [
              { committedDate: new Date().toISOString(), author: { name: 'Test' } },
              { committedDate: new Date(Date.now() - 86400000).toISOString(), author: { name: 'Test' } }
            ]
          }
        }
      },
...
MIT License`
  ...
  test('includes all expected breakdown metrics', () => {
    ...
    const expectedMetrics = [
      'readme', 'license', 'stars', 'forks', 'commits', 
      'workflows', 'issues', 'topics', 'languages', 'freshness'
    ];
  ...
  test('handles repository without README', () => {
    ...
    const result = calculateRepositoryScore(dataWithoutReadme, mockWorkflowData);
  ...
  test('handles repository without license', () => {
    ...
    const result = calculateRepositoryScore(dataWithoutLicense, mockWorkflowData);
  ...
  test('penalizes stale repositories', () => {
    const staleData = {
      ...
      repository: {
        ...mockRepoData.repository,
        pushedAt: new Date(Date.now() - 365 * 24 * 60 * 60 * 1000 * 2).toISOString() // 2 years ago
      }
    };
